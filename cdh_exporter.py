...
class JsonCollector(object):
	def __init__(self):
		pass
	def collect(self):
		namenode_url = "https://mgt2.hpe-project.com:9871/jmx" 
		#user = os.environ.get("USER")
		#pwd = os.environ.get("PASSWORD")
		# Fetch the JSON
		kerberos_auth = HTTPKerberosAuth(delegate=True)
		interim_response = requests.get(namenode_url, auth=(kerberos_auth),verify=False)
		response = interim_response.json()
		data = response["beans"]
...
class JsonCollector2(object):
	def __init__(self):
		pass
	def collect(self):
		yarn_url = "https://mgt2.hpe-project.com:8090/jmx"
		#user = os.environ.get("USER")
		#pwd = os.environ.get("PASSWORD")
		# Fetch the JSON
		kerberos_auth = HTTPKerberosAuth(delegate=True)
		interim_response = requests.get(yarn_url, auth=(kerberos_auth),verify=False)
		response = interim_response.json()
		data = response["beans"]
		...
		for i in data:
			...
			if i.get("LiveNodeManagers") != None:
				...
				metric.add_sample("num_live_node_managers", value=count, labels={"name": i.get("name")})
				yield metric
